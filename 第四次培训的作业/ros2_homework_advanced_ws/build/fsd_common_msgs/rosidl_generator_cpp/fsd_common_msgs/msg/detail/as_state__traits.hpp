...
// IWYU pragma: private, include "fsd_common_msgs/msg/as_state.hpp"
...
template<typename T, std::enable_if_t<std::is_same_v<std::decay_t<T>, fsd_common_msgs::msg::AsState>, int> = 0>
constexpr auto as_tuple_ref(T && msg)
{
  return std::forward_as_tuple(
    std::forward<T>(msg).header,
    std::forward<T>(msg).mission,
    std::forward<T>(msg).which_lap,
    std::forward<T>(msg).end,
    std::forward<T>(msg).finished,
    std::forward<T>(msg).camera_state,
    std::forward<T>(msg).lidar_state,
    std::forward<T>(msg).ins_state,
    std::forward<T>(msg).sensor_state,
    std::forward<T>(msg).ready,
    std::forward<T>(msg).count_time);
}
...
template<>
constexpr const char * data_type<fsd_common_msgs::msg::AsState>()
{
  return "fsd_common_msgs::msg::AsState";
}

template<>
constexpr const char * name<fsd_common_msgs::msg::AsState>()
{
  return "fsd_common_msgs/msg/AsState";
}
...
template<>
struct MessageTraits<fsd_common_msgs::msg::AsState>
{
