...
// IWYU pragma: private, include "fsd_common_msgs/msg/feedback.hpp"
...
template<typename T, std::enable_if_t<std::is_same_v<std::decay_t<T>, fsd_common_msgs::msg::Feedback>, int> = 0>
constexpr auto as_tuple_ref(T && msg)
{
  return std::forward_as_tuple(
    std::forward<T>(msg).header,
    std::forward<T>(msg).frame_id_num,
    std::forward<T>(msg).wheel_speed_fl,
    std::forward<T>(msg).wheel_speed_fr,
    std::forward<T>(msg).wheel_speed_rl,
    std::forward<T>(msg).wheel_speed_rr,
    std::forward<T>(msg).motor_command_speed_rl,
    std::forward<T>(msg).motor_command_speed_rr,
    std::forward<T>(msg).motor_return_speed_rl,
    std::forward<T>(msg).motor_return_speed_rr,
    std::forward<T>(msg).motor_command_torq_rl,
    std::forward<T>(msg).motor_command_torq_rr,
    std::forward<T>(msg).motor_return_torq_rl,
    std::forward<T>(msg).motor_return_torq_rr);
}
...
template<>
constexpr const char * data_type<fsd_common_msgs::msg::Feedback>()
{
  return "fsd_common_msgs::msg::Feedback";
}

template<>
constexpr const char * name<fsd_common_msgs::msg::Feedback>()
{
  return "fsd_common_msgs/msg/Feedback";
}
...
template<>
struct MessageTraits<fsd_common_msgs::msg::Feedback>
{
