...
// IWYU pragma: private, include "fsd_common_msgs/msg/asensing_message.hpp"
...
template<typename T, std::enable_if_t<std::is_same_v<std::decay_t<T>, fsd_common_msgs::msg::AsensingMessage>, int> = 0>
constexpr auto as_tuple_ref(T && msg)
{
  return std::forward_as_tuple(
    ...
    std::forward<T>(msg).heading_angle,
    ...
    std::forward<T>(msg).std_lat,
    std::forward<T>(msg).std_lon,
    std::forward<T>(msg).std_local_height,
    std::forward<T>(msg).north_velocity_std,
    std::forward<T>(msg).east_velocity_std,
    std::forward<T>(msg).ground_velocity_std,
    std::forward<T>(msg).roll_std,
    std::forward<T>(msg).pitch_std,
    std::forward<T>(msg).std_heading,
    std::forward<T>(msg).status,
    std::forward<T>(msg).gps_flag_pos,
    std::forward<T>(msg).gps_week_number,
    std::forward<T>(msg).ull_timestamp,
    std::forward<T>(msg).temperature,
    std::forward<T>(msg).car_status,
    std::forward<T>(msg).gps_flag_heading,
    std::forward<T>(msg).num_sv);
}
...
template<>
constexpr const char * data_type<fsd_common_msgs::msg::AsensingMessage>()
{
  return "fsd_common_msgs::msg::AsensingMessage";
}

template<>
constexpr const char * name<fsd_common_msgs::msg::AsensingMessage>()
{
  return "fsd_common_msgs/msg/AsensingMessage";
}
...
template<>
struct MessageTraits<fsd_common_msgs::msg::AsensingMessage>
{
  static constexpr std::size_t member_count = 34;
